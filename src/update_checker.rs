...
pub async fn check_for_update() -> anyhow::Result<()> {
    ...
    let raw_json: serde_json::Value = serde_json::from_str(&latest_release)?;

    // Getting the tag name
    let latest_version = raw_json["tag_name"].as_str().unwrap_or("0.0.0");

    let current_version = Version::from(VERSION).expect("Failed to parse current version!");
    let github_version = Version::from(latest_version).expect("Failed to parse GitHub version!");

    // Comparing the versions
    match current_version.compare(&github_version) {
        Cmp::Lt => {
            debug!("Newer version available! Current: {}, Latest: {}", current_version, github_version);
            info!("========================= \x1b[38;5;113mUPDATE\x1b[0m =========================");
